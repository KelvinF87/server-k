...
// Función auxiliar para convertir a inicio del día en UTC
function startOfDayUTC(dateString) {
    return moment.utc(dateString).startOf('day').toDate();
}

// Función auxiliar para convertir al final del día en UTC
function endOfDayUTC(dateString) {
    return moment.utc(dateString).endOf('day').toDate();
}
...
reportRouter.get("/alldata", isAuthenticated, async (req, res) => {
    try {
        ...
        // Convertir startDate y endDate a objetos Date, asegurando UTC
        const startDateObj = startOfDayUTC(startDate);
        const endDateObj = endOfDayUTC(endDate);
